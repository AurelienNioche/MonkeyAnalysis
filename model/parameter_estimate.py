...
def run(d):
    ...
    for monkey in monkeys:
        ...
        alternatives, n, k = data.filter.get_choose_risky(d[monkey])
        ...
        res = scipy.optimize.minimize(
            objective_multi, np.array([0, 0, 0.5, 0.5, 1, 1]), args=(alternatives, n, k, ), bounds=
            ((-0.99, 0.99), (-0.99, 0.99), (0.01, 1), (0.01, 1), (0, 5), (0, 5)))  # method=SLSQP
        ...
        r[monkey]["pos_distortion"] = pos_distortion
        r[monkey]["neg_precision"] = neg_precision
        r[monkey]["pos_precision"] = pos_precision

        log(f"{monkey}: parameters = {res.x}", name)
        log(f"{monkey}: log-likelihood sum = {res.fun}", name)
